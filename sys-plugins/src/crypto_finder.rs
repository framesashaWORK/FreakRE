...
const CRYPTO_CONSTANTS: &[(&[u8], &str, &str)] = &[
    ...
    (
        &[
            0x78, 0xA4, 0x6D, 0x24, 0x13, 0x72, 0xAE, 0xD6, 0x14, 0xB7, 0x4C, 0xCC, 0x99, 0x3B,
            0xA1, 0x3D,
        ],
        "blowfish_p",
        "Blowfish P",
    ...
    (
        &[
            0x18, 0x23, 0xC6, 0xE8, 0x87, 0xB8, 0x01, 0x4F, 0x36, 0xA6, 0xD2, 0xF5, 0x79, 0x6F,
            0x91, 0x52,
        ],
        "whirlpool_sbox",
        "Whirlpool S-Box",
...
impl Default for CryptoFinderPlugin {
    fn default() -> Self {
        Self
    }
}
...
impl Plugin for CryptoFinderPlugin {
    fn metadata(&self) -> PluginMetadata {
        PluginMetadata {
            ...
            description:
                "Scans for AES, SHA-256, MD5, DES, CRC32, Blowfish, RC4, ChaCha20 constants.".into(),
    ...
    fn menu_items(&self) -> Vec<MenuItem> {
        vec![
            MenuItem::new("Analyze/Crypto Constants", "Find Crypto Constants")
                .with_shortcut("Ctrl+Shift+C"),
        ]
    }
    fn on_menu_item(&mut self, ctx: &mut PluginContext, path: &str) {
        if path == "Analyze/Crypto Constants" {
            self.analyze(ctx);
        }
    }
    fn analyze(&mut self, ctx: &mut PluginContext) {
            ...
            for (pattern, label, name) in CRYPTO_CONSTANTS {
                ...
                for off in 0..=code.len() - pattern.len() {
                    if &code[off..off + pattern.len()] == *pattern {
                        ...
                        ctx.println(&format!(
                            "  ✓ {} at 0x{:X} in func 0x{:X}",
                            name, addr, func.address
                        ));
                        total += 1;
                        break;
                    }
        ...
        ctx.println(&format!(
            "[CryptoFinder] Done. Found {} crypto refs in {} functions.",
            total,
            functions.len()
        ));
    }
