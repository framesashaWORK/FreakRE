...
impl Lifter for MipsLifter {
    fn arch_name(&self) -> &str {
        if self.is_64bit { "mips64" } else { "mips32" }
    }
    ...
    fn lift_function(
        ...
        while offset + 4 <= code.len() {
            ...
            // Also try LE
            let instr_le = u32::from_le_bytes([code[offset], code[offset+1], code[offset+2], code[offset+3]]);
            // Heuristic: use BE if more plausible
            let word = if instr >> 26 == 0 && instr_le >> 26 != 0 { instr_le } else { instr };
            ...
            match opcode {
                0x00 => {
                    ...
                    match funct {
                        0x20 => { // add
                            let dst = self.reg(rd);
                            func.push_inst(current_block, IrInst::Binary { dst, op: OpCode::Add, lhs: self.reg(rs), rhs: self.reg(rt) });
                        }
                        0x21 => { // addu
                            let dst = self.reg(rd);
                            func.push_inst(current_block, IrInst::Binary { dst, op: OpCode::Add, lhs: self.reg(rs), rhs: self.reg(rt) });
                        ...
                        0x22 => { // sub
                            let dst = self.reg(rd);
                            func.push_inst(current_block, IrInst::Binary { dst, op: OpCode::Sub, lhs: self.reg(rs), rhs: self.reg(rt) });
                        }
                        0x24 => { // and
                            let dst = self.reg(rd);
                            func.push_inst(current_block, IrInst::Binary { dst, op: OpCode::And, lhs: self.reg(rs), rhs: self.reg(rt) });
                        }
                        0x25 => { // or
                            let dst = self.reg(rd);
                            func.push_inst(current_block, IrInst::Binary { dst, op: OpCode::Or, lhs: self.reg(rs), rhs: self.reg(rt) });
                        }
                        0x26 => { // xor
                            let dst = self.reg(rd);
                            func.push_inst(current_block, IrInst::Binary { dst, op: OpCode::Xor, lhs: self.reg(rs), rhs: self.reg(rt) });
                        }
                        0x08 => { // jr
                            func.push_inst(current_block, IrInst::IndirectBranch { target: self.reg(rs) });
                        }
                        0x09 => { // jalr
                            let dst = self.reg(rd);
                            func.push_inst(current_block, IrInst::Unary { dst, op: OpCode::Copy, src: Value::Const((address+8) as i64) });
                            func.push_inst(current_block, IrInst::Call { dst: Some(self.reg(31)), target: self.reg(rs), args: vec![] });
                        }
                        _ => {
                ...
                0x04 => { // beq
                    ...
                    func.push_inst(current_block, IrInst::Binary { dst: cond.clone(), op: OpCode::Eq, lhs: self.reg(rs), rhs: self.reg(rt) });
                    func.push_inst(current_block, IrInst::CBranch { cond, target_true: taken, target_false: not_taken });
                ...
                0x05 => { // bne
                    ...
                    let not_taken = func.add_block(&format!("fall_{:X}", address+4));
                    let cond = func.alloc_var(Ty::Bool);
                    func.push_inst(current_block, IrInst::Binary { dst: cond.clone(), op: OpCode::Ne, lhs: self.reg(rs), rhs: self.reg(rt) });
                    func.push_inst(current_block, IrInst::CBranch { cond, target_true: taken, target_false: not_taken });
                    current_block = not_taken;
                }
                0x08 | 0x09 => { // addi/addiu
                    let dst = self.reg(rt);
                    func.push_inst(current_block, IrInst::Binary { dst, op: OpCode::Add, lhs: self.reg(rs), rhs: Value::Const(imm) });
                }
                0x0C => { // andi
                    let dst = self.reg(rt);
                    func.push_inst(current_block, IrInst::Binary { dst, op: OpCode::And, lhs: self.reg(rs), rhs: Value::Const(imm & 0xFFFF) });
                }
                0x0D => { // ori
                    let dst = self.reg(rt);
                    func.push_inst(current_block, IrInst::Binary { dst, op: OpCode::Or, lhs: self.reg(rs), rhs: Value::Const(imm & 0xFFFF) });
                }
                0x0F => { // lui
                    let dst = self.reg(rt);
                ...
                0x23 => { // lw
                    ...
                    func.push_inst(current_block, IrInst::Binary { dst: addr.clone(), op: OpCode::Add, lhs: self.reg(rs), rhs: Value::Const(imm) });
                    func.push_inst(current_block, IrInst::Load { dst, addr, size: 4 });
                }
                0x2B => { // sw
                    let addr = func.alloc_var(if self.is_64bit { Ty::i64() } else { Ty::i32() });
                    func.push_inst(current_block, IrInst::Binary { dst: addr.clone(), op: OpCode::Add, lhs: self.reg(rs), rhs: Value::Const(imm) });
                    func.push_inst(current_block, IrInst::Store { addr, value: self.reg(rt), size: 4 });
                }
                _ => {
                    func.push_inst(current_block, IrInst::Nop);
                }
            ...
            if let Some(block) = func.block(current_block) {
                if block.terminator().is_some() && offset + 4 < code.len() {
                    ...
                    let has_fallthrough = matches!(block.terminator(), Some(IrInst::CBranch { .. }));
                    if has_fallthrough {
                        // CBranch already has fallthrough, current_block is already set to not_taken
                    } else if matches!(block.terminator(), Some(IrInst::Branch { .. }) | Some(IrInst::IndirectBranch { .. })) {
                        // Unconditional - need new block for next insn if not already
                        if func.blocks.iter().any(|b| b.label == format!("bb_{}", offset+4)) {
                            // reuse?
                        } else {
                            current_block = func.add_block(&format!("bb_{}", offset+4));
                        }
