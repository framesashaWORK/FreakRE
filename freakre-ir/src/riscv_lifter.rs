...
impl RiscvLifter {
    pub fn new(is_64bit: bool) -> Self {
        Self { is_64bit, max_instructions: 100_000 }
    }
    fn reg(&self, idx: u8) -> Value {
        let names = ["zero","ra","sp","gp","tp","t0","t1","t2","s0","s1","a0","a1","a2","a3","a4","a5","a6","a7","s2","s3","s4","s5","s6","s7","s8","s9","s10","s11","t3","t4","t5","t6"];
        let name = names.get(idx as usize).copied().unwrap_or("x?");
        Value::Register { name: name.to_string(), ty: if self.is_64bit { Ty::i64() } else { Ty::i32() } }
    }
}

impl Lifter for RiscvLifter {
    fn arch_name(&self) -> &str { if self.is_64bit { "riscv64" } else { "riscv32" } }
    fn max_instructions(&self) -> usize { self.max_instructions }
    fn lift_function(&self, code: &[u8], base_address: u64, function_name: &str) -> Result<IrFunction, LifterError> {
        ...
        while offset + 4 <= code.len() {
            ...
            let imm_s = if imm_s & 0x800 != 0 { imm_s | !0xFFF } else { imm_s };
            let imm_b = {
                ...
                if imm & 0x1000 != 0 { (imm | !0x1FFF) as i64 } else { imm as i64 }
            };
            ...
            let imm_j = {
                ...
                if imm & 0x100000 != 0 { (imm | !0x1FFFFF) as i64 } else { imm as i64 }
            };
            ...
            match opcode {
                0x33 => { // R-type
                    ...
                    let op = match (funct3, funct7) {
                        ...
                        (5, 0x00) => OpCode::Shr,
                        (5, 0x20) => OpCode::Sar,
                        _ => { func.push_inst(current_block, IrInst::Nop); offset+=4; continue; }
                    };
                    func.push_inst(current_block, IrInst::Binary { dst, op, lhs: self.reg(rs1), rhs: self.reg(rs2) });
                }
                0x13 => { // I-type ALU
                    let dst = self.reg(rd);
                    let op = match funct3 {
                        ...
                        7 => OpCode::And,
                        1 => OpCode::Shl,
                        5 => if (word >> 30) & 1 == 1 { OpCode::Sar } else { OpCode::Shr },
                        _ => { func.push_inst(current_block, IrInst::Nop); offset+=4; continue; }
                    };
                    func.push_inst(current_block, IrInst::Binary { dst, op, lhs: self.reg(rs1), rhs: Value::Const(imm_i) });
                }
                0x03 => { // Loads
                    let dst = self.reg(rd);
                    let size = match funct3 { 0|4 => 1, 1|5 => 2, 2 => 4, 3 => 8, _ => 4 };
                    let addr = func.alloc_var(if self.is_64bit { Ty::i64() } else { Ty::i32() });
                    func.push_inst(current_block, IrInst::Binary { dst: addr.clone(), op: OpCode::Add, lhs: self.reg(rs1), rhs: Value::Const(imm_i) });
                    func.push_inst(current_block, IrInst::Load { dst, addr, size });
                }
                0x23 => { // Stores
                    let size = match funct3 { 0 => 1, 1 => 2, 2 => 4, 3 => 8, _ => 4 };
                    let addr = func.alloc_var(if self.is_64bit { Ty::i64() } else { Ty::i32() });
                    func.push_inst(current_block, IrInst::Binary { dst: addr.clone(), op: OpCode::Add, lhs: self.reg(rs1), rhs: Value::Const(imm_s) });
                    func.push_inst(current_block, IrInst::Store { addr, value: self.reg(rs2), size });
                }
                0x63 => { // Branches
                    ...
                    let op = match funct3 { 0 => OpCode::Eq, 1 => OpCode::Ne, 4 => OpCode::LtS, 5 => OpCode::GeS, 6 => OpCode::LtU, 7 => OpCode::GeU, _ => OpCode::Eq };
                    func.push_inst(current_block, IrInst::Binary { dst: cond.clone(), op, lhs: self.reg(rs1), rhs: self.reg(rs2) });
                ...
                0x67 => { // jalr
                    let dst = self.reg(rd);
                    if rd != 0 {
                        func.push_inst(current_block, IrInst::Unary { dst: dst.clone(), op: OpCode::Copy, src: Value::Const((address+4) as i64) });
                    }
                    let target = func.alloc_var(if self.is_64bit { Ty::i64() } else { Ty::i32() });
                    func.push_inst(current_block, IrInst::Binary { dst: target.clone(), op: OpCode::Add, lhs: self.reg(rs1), rhs: Value::Const(imm_i) });
                    func.push_inst(current_block, IrInst::IndirectBranch { target });
                }
                0x37 => { // lui
                    let dst = self.reg(rd);
                    func.push_inst(current_block, IrInst::Unary { dst, op: OpCode::Copy, src: Value::Const(imm_u) });
                }
                0x17 => { // auipc
                    let dst = self.reg(rd);
            ...
            if let Some(b) = func.block(current_block) {
                if b.terminator().is_some() && offset + 4 < code.len() {
                    ...
                    if !func.blocks.iter().any(|b| b.label == next_label) {
                        ...
                        let term = func.block(current_block).and_then(|b| b.terminator().cloned());
