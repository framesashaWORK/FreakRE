...
pub fn analyze_dotnet(data: &[u8]) -> Option<DotnetReport> {
    ...
    if opt_size < 112 || opt_hdr_off + opt_size > data.len() {
        return None;
    }

    // Number of data directories (PE32: at +96, PE32+: at +112)
    let num_dd_off = if is_pe32_plus {
        opt_hdr_off + 108
    } else {
        opt_hdr_off + 92
    };
    ...
    let dd_off = if is_pe32_plus {
        opt_hdr_off + 112
    } else {
        opt_hdr_off + 96
    };
    if dd_off + 15 * 8 > data.len() {
        return None;
    }
    ...
    if sect_off + num_sect * 40 > data.len() {
        return None;
    }
    ...
    if md_off + metadata_size > data.len() {
        return None;
    }
    ...
    if flags_raw & 0x01 != 0 {
        flags.push("ILONLY".into());
    }
    if flags_raw & 0x02 != 0 {
        flags.push("32BITREQUIRED".into());
    }
    if flags_raw & 0x04 != 0 {
        flags.push("IL_LIBRARY".into());
    }
    if flags_raw & 0x08 != 0 {
        flags.push("STRONGNAMESIGNED".into());
    }
    if flags_raw & 0x10 != 0 {
        flags.push("NATIVE_ENTRYPOINT".into());
    }
    if flags_raw & 0x20 != 0 {
        flags.push("TRACKDEBUGDATA".into());
    }
    ...
    if native_entry_rva != 0 {
        push_finding(
            &mut findings,
            DotnetSeverity::High,
            "DOTNET_NATIVE_ENTRYPOINT",
            "CLR has a native entry point — often a managed→native loader".into(),
            clr_off,
        );
    }
    if vtable_fixups_rva != 0 {
        push_finding(
            &mut findings,
            DotnetSeverity::Medium,
            "DOTNET_VTABLE_FIXUPS",
            "CLR contains v-table fixups — COM interop or unmanaged callbacks".into(),
            clr_off,
        );
    }
        ...
        for s in &streams {
            if s.name == "#~" {
                ...
                if let Some(names) = parse_strings_heap(md, s.offset, s.size) {
                    ...
                    if let Some(m) = names
                        .iter()
                        .find(|n| n.ends_with(".dll") && !n.starts_with("System."))
                    {
    ...
    if report.member_ref_count > 0 {
        let suspicious_refs: Vec<&String> = report
            .member_refs_sample
            .iter()
            .filter(|s| {
                s.contains("Process")
                    || s.contains("Shell")
                    || s.contains("WinExec")
                    || s.contains("VirtualAlloc")
                    || s.contains("LoadLibrary")
                    || s.contains("GetProcAddress")
                    || s.contains("CreateThread")
                    || s.contains("WriteProcessMemory")
                    || s.contains("QueueUserAPC")
                    || s.contains("SetWindowsHookEx")
                    || s.contains("InternetOpen")
                    || s.contains("HttpSendRequest")
                    || s.contains("RegSetValueEx")
                    || s.contains("AdjustTokenPrivileges")
                    || s.contains("OpenProcessToken")
                    || s.contains("LookupPrivilegeValue")
            })
            .collect();
        if !suspicious_refs.is_empty() {
            push_finding(
                ...
                format!(
                    "Suspicious native API references: {}",
                    suspicious_refs
                        .iter()
                        .take(5)
                        .map(|s| s.as_str())
...
fn push_finding(
    out: &mut Vec<DotnetFinding>,
    severity: DotnetSeverity,
    rule_id: &str,
    description: String,
    offset: usize,
) {
    out.push(DotnetFinding {
        severity,
...
fn parse_tables_stream(md: &[u8], off: usize, size: usize) -> Option<TablesInfo> {
    ...
    if let Some((streams, _)) = parse_stream_dir(md) {
        ...
        if let Some(strs) = strings_stream {
            ...
            info.assembly_refs = all
                .iter()
                .filter(|n| n.ends_with(".dll") || n.ends_with(".exe"))
                .take(64)
                .cloned()
                .collect();
            info.type_refs_sample = all
                .iter()
                .filter(|n| !n.is_empty() && n.contains('.'))
                .take(128)
                .cloned()
                .collect();
            info.member_refs_sample = all
                .iter()
                .filter(|n| {
                    n.contains('(')
                        || n.contains("Get")
...
fn parse_user_strings(md: &[u8], off: usize, size: usize) -> Option<UserStrings> {
    ...
    while p < end {
        ...
        if len >= 2 {
            ...
            if !text.is_empty() {
                out.count += 1;
                if out.sample.len() < 200 {
                    out.sample.push(text.clone());
                }
